 ...
 */
 ...
 function local_customtransmute_log(string $message, bool $iserror = false): void {
     ...
     if ($iserror) {
         debugging("CustomTransmute ERROR: {$message}", DEBUG_DEVELOPER);
         mtrace("CustomTransmute ERROR: {$message}");
     } else {
         debugging("CustomTransmute: {$message}", DEBUG_DEVELOPER);
         mtrace("CustomTransmute: {$message}");
     }
 }
